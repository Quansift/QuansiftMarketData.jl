...
@testset "Checkpoints fire on attempts, so failures cannot starve them" begin
    # The first version of this fix keyed the checkpoint off the SUCCESS count.
    # In production that deadlocked: memory pressure made the upserts fail, the
    # success count stopped advancing at 461 against a 500 interval, not one
    # checkpoint ever ran, and ~95% of securities were lost to OOM. Counting
    # attempts is the property that matters, so assert it by making every
    # upsert fail and checking the run still completes instead of stalling.
    as_of = Date(2026, 7, 19)
    fetched_at = DateTime(2026, 7, 19, 12)
    security_count = 6
    ...
    try
        create_tables(conn)
        # Move the upsert target aside so every write throws, standing in for
        # the OOM failures seen in production. In-memory database only.
        DBInterface.execute(
            conn, "ALTER TABLE fundamental_daily_metrics RENAME TO metrics_parked",
        )
        result = sync_fundamentals!(
            conn, meta_payload, universe_payload;
            api_key = "offline-token", as_of, history_years = 3,
        ...
        @test length(result.failed) == security_count
        @test isempty(result.requested)
        @test result.metric_rows == 0
    finally
        close_duckdb(conn)
